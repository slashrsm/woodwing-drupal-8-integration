...
class Drupal8_AdminProperties extends AdminProperties_EnterpriseConnector
{
		...
		switch( $entity ) {
			...
			case 'PubChannel':
				...
				if( $mode == 'draw_dialog' ) { // Show separator on dialogs, but do not add it to the DB model.
					$widgets[WW_Plugins_Drupal8_Utils::CHANNEL_SEPERATOR] = new DialogWidget(
						new PropertyInfo( WW_Plugins_Drupal8_Utils::CHANNEL_SEPERATOR, 'Drupal Account', null, 'separator' ),
	...
	private function isCorrectPublishSystem($context, $entity, $action)
	{
		...
		if( $entity == 'PubChannel' && $action != 'Create') {
			...
			if ($action == 'Update') {
				...
				if ($publicationChannel->Type != 'web' || $publicationChannel->PublishSystem != WW_Plugins_Drupal8_Utils::DRUPAL8_PLUGIN_NAME) {
			...
			if( $chanType == 'web' && $publishSystem == WW_Plugins_Drupal8_Utils::DRUPAL8_PLUGIN_NAME && !$isSystemChanged) {
				$match = true;
			}
		}
		return $match;
